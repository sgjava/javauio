...
```
So one thing to note is JDK 25 on ARM32 seems to be much slower than JDK 11. Lets switch over to JDK 11:
```
...
it's overkill and you'll want to stick with JDK 25. For now I'm keeping the code
compatible with JDK11, so it's really just a compile time issue. Using an oscilloscope
is the only way to know the true speed. In this case JDK 25 allowed around 900 KHz
GPIO toggle.
